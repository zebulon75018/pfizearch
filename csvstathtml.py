...
class CSVStat(CSVKitUtility):
    ...
    def get_one(self, table, column_id, operation, label=True, **kwargs):
        """
    ...
    def print_one(self, table, column_id, operation, label=True, **kwargs):
        stat = self.get_one(table, column_id, operation, label, kwargs)
        column_name = table.column_names[column_id]
    ...
    def print_stats(self, table, column_ids, stats):
        ...
        for column_id in column_ids:
            if  column_id not in CSVStat.jsonresult:
                CSVStat.jsonresult[column_id] = {}

            column_name = table.column_names[column_id]
            column = table.columns[column_id]
            column_stats = stats[column_id]

            for n,f in  enumerate(column_stats['freq']):
                column_stats['freq'][n]["value"] = str(column_stats['freq'][n]["value"])


            if self.args.json_only is False:
                  self.output_file.write(('%3i. "%s"\n\n' % (column_id + 1, column_name)))
            CSVStat.jsonresult[column_id] = {"column_name":column_name,"column_stats":column_stats}
        ...
        CSVStat.jsonresult["row_count"]=len(table.rows)
        if self.args.json_only is False:
...
def launch_new_instance():
    utility = CSVStat()
    utility.run()
    if utility.args.json_only:
        print(json.dumps(CSVStat.jsonresult,indent=4))
